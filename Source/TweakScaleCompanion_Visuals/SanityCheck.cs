...
namespace TweakScaleCompanion.Visuals
{
	...
	internal class SanityCheck : MonoBehaviour
	{
		...
		private IEnumerator SanityCheckCoroutine()
		{
			...
			foreach (AvailablePart p in PartLoader.LoadedPartsList)
			{
				...
				{ 
					...
					{ 
						...
						try 
						{
							containsInventoryScaler = prefab.Modules.Contains(SCALER_MODULE_NAME);
							++total_count;
			...
			Log.info("SanityCheck Concluded : {0} parts found ; {1} parts using {2} ; {3} show stoppers detected .", total_count, parts_with_inventory_count, TARGET_MODULE_NAME, showstoppers_count);
		...
		private string checkForWaterfall(Part p)
		{
			...
			if (!p.Modules.Contains(TARGET_MODULE_NAME)) return MSG_PART_DOES_NOT_NEED;
